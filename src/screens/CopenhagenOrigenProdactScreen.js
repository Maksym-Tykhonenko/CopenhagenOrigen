...
const CopenhagenOrigenProdactScreen = ({navigation, route}) => {
  ...
  const INITIAL_URL_KEY = `https://great-glorious-jubilation.space/2FP9whbb`;
  ...
  const setData = async () => {
    try {
      ...
      await AsyncStorage.setItem('CopenhagenOrigenProdactScreen', jsonData);
  ...
  ////////////////////////////
  let baseUrl = `${INITIAL_URL_KEY}?2FP9whbb=1&advertising_id=${idfa}&uid=${uid}&adAtribution=${adAtribution}&adKeywordId=${adKeywordId}&customer_user_id=${customerUserId}&idfv=${idfv}&onesignal_id=${oneSignalId}`;
...
export default CopenhagenOrigenProdactScreen;
